    ...
    <section className="py-16 md:py-24 bg-background relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-12">
          <p className="text-orange-600 dark:text-orange-400 text-sm font-bold uppercase tracking-widest mb-3">Reviews</p>
          <h2
          ...
          <div className="flex items-center justify-center gap-2 mt-6">
            <div className="flex gap-1">
              {Array.from({ length: 5 }).map((_, i) => (
                <Star key={i} className="h-5 w-5 fill-amber-600 dark:fill-amber-400 text-amber-600 dark:text-amber-400" />
              ))}
            </div>
            <span className="text-foreground font-bold text-lg">4.9</span>
            <span className="text-muted-foreground/40 text-sm">/ 5 (2,847 reviews)</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {testimonials.map((t, index) => (
            <div
              key={index}
              className="group relative bg-muted/50 hover:bg-muted border border-border hover:border-orange-500/20 rounded-2xl p-5 md:p-6 transition-all duration-300 hover:-translate-y-0.5"
            >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <div>
                      ...
                      {t.verified && (
                        <span className="ml-2 text-xs bg-green-500/15 text-green-600 dark:text-green-400 border border-green-500/20 px-1.5 py-0.5 rounded-full">
                          ✓ Verified
                        </span>
                      )}
                    </div>
                    <div className="flex gap-0.5">
                      {Array.from({ length: t.rating }).map((_, i) => (
                        <Star key={i} className="h-3.5 w-3.5 fill-amber-600 dark:fill-amber-400 text-amber-600 dark:text-amber-400" />
                      ))}
                    </div>
                  </div>

                  <div className="flex items-center gap-2 mt-0.5 mb-3">
                    <span className="text-xs text-muted-foreground/40">📍 {t.location}</span>
                    <span className="text-xs text-muted-foreground/30">·</span>
                    <span className="text-xs text-muted-foreground/40">{t.sport}</span>
                  </div>

                  {/* Khmer text (if any) */}
                  {t.textEn ? (
                    <div>
                      <p className="text-muted-foreground/70 text-sm leading-relaxed mb-1">&ldquo;{t.text}&rdquo;</p>
                      <p className="text-muted-foreground/40 text-xs italic">{t.textEn}</p>
                    </div>
                  ) : (
                    <p className="text-muted-foreground/70 text-sm leading-relaxed">&ldquo;{t.text}&rdquo;</p>
                  )}
                </div>
              </div>
            </div>
        ...
        <div className="mt-12 flex flex-col sm:flex-row items-center justify-center gap-4 text-center">
          <div className="bg-[#0088cc]/15 border border-[#0088cc]/30 rounded-2xl px-6 py-4 flex items-center gap-4">
            ...
            <div className="text-left">
              <div className="text-foreground font-bold text-sm">Chat with us on Telegram</div>
              <div className="text-muted-foreground/40 text-xs mt-0.5">Get instant help, order tracking & deals</div>
            </div>
            <a
