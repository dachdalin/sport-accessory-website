...
function Row({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex items-center justify-between gap-4 py-3 border-b border-border/50 last:border-0">
      ...
      <span className={`text-sm font-semibold ${highlight ? "text-green-600 dark:text-green-400" : "text-foreground"}`}>{value}</span>
    </div>
  )
}
        ...
        <main className="flex-1">
          ...
          <section className="relative border-b border-border bg-card px-4 py-16 text-center md:py-24 overflow-hidden">
            ...
            <div className="relative">
              ...
              <h1 className="font-heading text-3xl font-bold text-foreground md:text-5xl" style={{ fontFamily: "var(--font-heading)" }}>
                Shipping Policy
              </h1>
          ...
          <section className="px-4 py-14 md:py-20">
            <div className="mx-auto max-w-3xl space-y-6">
              ...
              {/* Shipping Fees */}
              <SectionCard icon={<CreditCard className="h-5 w-5 text-orange-600 dark:text-orange-400" />} title="Shipping Fees">
                <div className="rounded-xl border border-border overflow-hidden">
                  <div className="grid grid-cols-3 bg-muted/50 px-4 py-2.5 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                  ...
                  {[
                    ...
                    <div key={range} className="grid grid-cols-3 border-t border-border/50 px-4 py-3.5">
                      <span className="text-sm text-muted-foreground">{range}</span>
                      <span className={`text-sm font-semibold text-center ${capital === "FREE" ? "text-green-600 dark:text-green-400" : "text-foreground"}`}>
                        {capital}
                      </span>
                      <span className={`text-sm font-semibold text-right ${province === "FREE" ? "text-green-600 dark:text-green-400" : "text-foreground"}`}>
                        {province}
                      </span>
                    </div>
                  ))}
              ...
              {/* Delivery Time */}
              <SectionCard icon={<Clock className="h-5 w-5 text-orange-600 dark:text-orange-400" />} title="Delivery Time">
              ...
              {/* Order Processing */}
              <SectionCard icon={<Package className="h-5 w-5 text-orange-600 dark:text-orange-400" />} title="Order Processing">
                <div className="space-y-0">
              ...
              {/* Delivery Coverage */}
              <SectionCard icon={<MapPin className="h-5 w-5 text-orange-600 dark:text-orange-400" />} title="Delivery Coverage">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {[
                    ...
                    <div key={province} className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              {/* Delivery Partners */}
              <SectionCard icon={<Truck className="h-5 w-5 text-orange-600 dark:text-orange-400" />} title="Delivery Partners">
              ...
              <SectionCard icon={<Banknote className="h-5 w-5 text-orange-600 dark:text-orange-400" />} title="Payment & Cash on Delivery (COD)">
                <p className="text-sm text-muted-foreground mb-5 leading-6">
                ...
                <div className="flex items-start gap-2.5 rounded-xl bg-green-500/8 border border-green-500/15 px-4 py-3">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Cash on Delivery is available for all orders up to <span className="text-foreground font-medium">$200</span>.
              ...
              {/* Order Tracking */}
              <SectionCard icon={<Zap className="h-5 w-5 text-orange-600 dark:text-orange-400" />} title="Order Tracking">
              ...
              <SectionCard icon={<RefreshCcw className="h-5 w-5 text-orange-600 dark:text-orange-400" />} title="Failed Delivery & Returns">
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <div key={step} className="flex gap-3">
                      <div className="h-6 w-6 rounded-full bg-orange-500/20 text-orange-600 dark:text-orange-400 text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5">
                        {step[0]}
                      </div>
                      <div>
              ...
              {/* Contact Support */}
              <SectionCard icon={<Phone className="h-5 w-5 text-orange-600 dark:text-orange-400" />} title="Shipping Support">
                <p className="text-sm text-muted-foreground leading-6 mb-5">
