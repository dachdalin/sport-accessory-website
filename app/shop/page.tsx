  ...
  return (
    <div className="px-1 pt-2 pb-4">
      ...
      <div className="flex justify-between mt-3">
        <span className="text-xs bg-white/8 border border-white/10 text-white/70 px-2.5 py-1 rounded-lg font-mono font-medium">${value[0]}</span>
        <span className="text-xs bg-white/8 border border-white/10 text-white/70 px-2.5 py-1 rounded-lg font-mono font-medium">${value[1]}</span>
      </div>
    </div>
  )
}
...
function FilterPanel({
  ...
  return (
    <div className="space-y-6">
      {/* Categories */}
      <div>
        <h3 className="text-xs font-black text-white/40 uppercase tracking-widest mb-3" style={{ fontFamily: "var(--font-heading)" }}>
          Category
        </h3>
        <div className="space-y-1">
          {allCategories.map((cat) => (
            ...
            >
              ...
              {selectedCategory === cat.id && <Check className="h-3.5 w-3.5 text-orange-400" />}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-semibold text-white" style={{ fontFamily: "var(--font-heading)" }}>In Stock Only</h3>
          <p className="text-xs text-white/40 mt-0.5">Hide out-of-stock items</p>
        </div>
        <button
          onClick={() => setInStockOnly(!inStockOnly)}
          className={`relative w-11 h-6 rounded-full transition-colors ${inStockOnly ? "bg-orange-500" : "bg-white/10"}`}
        >
      ...
      <button
        onClick={onClear}
        className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl border border-white/10 text-white/40 hover:border-orange-500/30 hover:text-orange-400 text-sm font-medium transition-all"
      >
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#050A14]">
      ...
      <main className="flex-1">
        ...
        {/* ── Shop Hero Banner ─────────────────────────────────────────────── */}
        <div className="relative bg-[#080D18] border-b border-white/5 overflow-hidden">
          {/* Glow accents */}
          ...
          <div className="container mx-auto px-4 py-10 md:py-14 relative z-10">
            <div className="max-w-2xl">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-orange-400 text-xs font-black uppercase tracking-widest">Sport Accessories</span>
                <span className="text-white/20">·</span>
                <span className="text-white/40 text-xs">{products.length} products</span>
              </div>
              <h1
              ...
              <div className="relative mt-6 max-w-md">
                ...
                <input
                  ...
                  id="shop-search"
                  className="w-full bg-white/6 border border-white/10 text-white placeholder-white/30 text-sm pl-10 pr-10 py-3 rounded-xl focus:outline-none focus:border-orange-500/50 focus:bg-white/8 transition-all"
                />
                {searchQuery && (
                  <button onClick={() => setSearchQuery("")} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-white/30 hover:text-white/60">
                    <X className="h-4 w-4" />
                  </button>
                )}
              </div>
        ...
        <div className="bg-[#080D18] border-b border-white/5 sticky top-16 z-30">
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="flex gap-6 lg:gap-8">
            ...
            <aside className="hidden lg:block w-64 xl:w-72 flex-shrink-0">
              <div className="sticky top-32 bg-white/3 border border-white/8 rounded-2xl p-5">
                <div className="flex items-center justify-between mb-5">
                  <h2 className="text-sm font-black text-white uppercase tracking-wider flex items-center gap-2" style={{ fontFamily: "var(--font-heading)" }}>
                    <SlidersHorizontal className="h-4 w-4 text-orange-400" />
                    Filters
                  </h2>
                  {activeFilterCount > 0 && (
                    <span className="text-xs bg-orange-500/20 border border-orange-500/30 text-orange-400 px-2 py-0.5 rounded-full font-bold">
              ...
              <div className="flex items-center justify-between mb-5 gap-3 flex-wrap">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <button
                    ...
                    className="lg:hidden flex items-center gap-2 bg-white/6 border border-white/10 hover:border-orange-500/30 text-white/70 hover:text-white text-sm font-semibold px-3.5 py-2 rounded-xl transition-all"
                  >
                  ...
                  {selectedCategory !== "all" && (
                    <button
                      ...
                      className="flex items-center gap-1 bg-orange-500/10 border border-orange-500/25 text-orange-400 text-xs font-semibold px-2.5 py-1 rounded-full hover:bg-orange-500/20 transition-all"
                    >
                  ...
                  {minRating > 0 && (
                    <button
                      ...
                      className="flex items-center gap-1 bg-amber-500/10 border border-amber-500/25 text-amber-400 text-xs font-semibold px-2.5 py-1 rounded-full hover:bg-amber-500/20 transition-all"
                    >
                  ...
                  {(priceRange[0] > 0 || priceRange[1] < 200) && (
                    <button
                      ...
                      className="flex items-center gap-1 bg-blue-500/10 border border-blue-500/25 text-blue-400 text-xs font-semibold px-2.5 py-1 rounded-full hover:bg-blue-500/20 transition-all"
                    >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div ref={sortRef} className="relative">
                    <button
                      ...
                      className="flex items-center gap-2 bg-white/6 border border-white/10 hover:border-white/20 text-white/70 hover:text-white text-sm font-semibold px-3.5 py-2 rounded-xl transition-all"
                    >
                    ...
                    {sortOpen && (
                      <div className="absolute right-0 top-full mt-2 w-52 bg-[#0D1525] border border-white/12 rounded-xl shadow-2xl shadow-black/50 overflow-hidden z-40">
                        {sortOptions.map((opt) => (
                          <button
                            key={opt.value}
                  ...
                  <div className="flex bg-white/6 border border-white/10 rounded-xl overflow-hidden">
                    <button
                      ...
                      className={`p-2.5 transition-all ${viewMode === "grid" ? "bg-orange-500/20 text-orange-400" : "text-white/40 hover:text-white/70"}`}
                    ...
                    <button
                      ...
                      className={`p-2.5 transition-all ${viewMode === "list" ? "bg-orange-500/20 text-orange-400" : "text-white/40 hover:text-white/70"}`}
      ...
      {/* Drawer */}
      <div
        className={`fixed top-0 left-0 bottom-0 z-[70] w-[300px] bg-[#080D18] border-r border-white/10 flex flex-col shadow-2xl transition-transform duration-300 ease-out lg:hidden ${
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/8">
          <div className="flex items-center gap-2">
            <SlidersHorizontal className="h-4 w-4 text-orange-400" />
            <h2 className="font-black text-white" style={{ fontFamily: "var(--font-heading)" }}>Filters</h2>
            {activeFilterCount > 0 && (
              <span className="text-xs bg-orange-500/20 border border-orange-500/30 text-orange-400 px-2 py-0.5 rounded-full font-bold">
                {activeFilterCount}
              </span>
